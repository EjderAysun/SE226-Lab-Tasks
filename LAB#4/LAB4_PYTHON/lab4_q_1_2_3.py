...
while(True):
    greater_number = int(input("Please enter a number greater than the one you just entered and smaller than 999(inclusive): "))
    if(greater_number > smaller_number and greater_number <= 999):
        break
    else:
        print("Invalid input.")

sample_space = words[smaller_number : greater_number : 1]
#                       included       not included
# Traditionally we can create the list also using a “for loop”:
'''
for i in range(smaller_number, greater_number):
...
for i in range(user_number):
    event.append(random.choice(sample_space))
'''
